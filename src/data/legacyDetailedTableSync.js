  ...
  return `
    <div class="card" data-legacy-detailed-log style="padding:0;overflow:hidden;margin-top:14px">
      ...
      <div style="overflow-x:auto;-webkit-overflow-scrolling:touch">
        <table class="tbl" style="width:100%;min-width:850px;border-collapse:collapse">
          <thead>
            <tr><th>#</th><th style="text-align:left">Franchise</th><th class="num">P</th><th class="num">W</th><th class="num">L</th><th class="num">Pts</th><th class="num">PF</th><th class="num">PA</th><th class="num">GD</th><th class="num">SW</th><th class="num">SL</th><th class="num">SD</th></tr>
          </thead>
          <tbody>
            ${LEGACY_DETAILED_STANDINGS.map((row, index) => {
              const fr = legacyFranchiseById(row.franchise_id);
              if (!fr) return '';
              return `<tr>
                <td class="num"><b>${index + 1}</b></td>
                <td><a href="/legacy-franchise/${fr.id}" style="display:flex;align-items:center;gap:8px;text-decoration:none;color:inherit"><img src="${fr.logo}" alt="" style="width:28px;height:28px;object-fit:contain"><b>${fr.name}</b></a></td>
                ...
                <td class="num"><b>${signed(row.gd)}</b></td>
                <td class="num">${row.sw}</td>
...
function syncLegacyDetailedLog() {
  ...
  if (location.pathname === '/leagues') {
    ...
    if (!page.querySelector('[data-legacy-detailed-log]')) {
      ...
      const legacyTable = tables.find((table) => /Franchise/i.test(table.textContent || '') && /GD/i.test(table.textContent || ''));
      if (legacyTable) {
        const shell = legacyTable.closest('.card, .league-standings-shell') || legacyTable.parentElement;
        shell?.insertAdjacentHTML('afterend', buildDetailedTable());
      } else {
  ...
  if (location.pathname === '/rankings') {
    ...
    if (!page.querySelector('[data-legacy-detailed-log]')) {
      ...
      (table?.closest('.card') || page.lastElementChild)?.insertAdjacentHTML('afterend', buildDetailedTable());
    }
  }
}
